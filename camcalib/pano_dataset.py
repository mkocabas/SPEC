...
class CameraRegressorDataset(Dataset):
    def __init__(
            self,
        ...
        self.dataset_folder = DATASET_FOLDERS[dataset]
    ...
    def __getitem__(self, index):
        ...
        # norm_img = self.data_transform(image=cv_img)['image']
        # norm_img = np.transpose(norm_img.astype('float32'), (2, 0, 1))
        ...
        elif self.dataset == 'pano_scalenet':
            data = json.load(open(imgname.replace('.jpg', '.json')))
            pitch = data['pitch']  # in radians
            roll = data['roll']  # in radians
            vfov = data['vfov']  # in radians
        else:
            raise ValueError(f'{self.dataset} is not implemented.')
...
def test_dataset():
    ...
    ds = CameraRegressorDataset(dataset='pano_scalenet', is_train=False, loss_type='softargmax_l2')

    print('Dataset length: ', len(ds))
    dl = DataLoader(dataset=ds, batch_size=1, shuffle=False, collate_fn=collator)
